...
####################################################################################
#
# Modified version to better support revision control.
# Can be called as write_project_tcl_git from the tcl console in Vivado.
#
# Differences:
#
...
#
#   6. Recreated block designs will not contain their layout information.
#
####################################################################################
...
proc add_references { sub_design } {
  ...
  foreach file $refs {
    if { [file extension $file ] ==".bd" } {
      ...
      # Skip adding file if it's already part of the project, but only
      # add references if the sources are inside the project directory
      set file_rel_path [get_relative_file_path_for_source $file [get_script_execution_dir]]
      if { [string first .. $file_rel_path] == -1 } {
        lappend l_script_data "if { \[get_files [file tail $file_rel_path]\] == \"\" } {"
        lappend l_script_data "  import_files -quiet -fileset [current_fileset -srcset] \"\$origin_dir/$file_rel_path\"\n}"
      }
    }
  }
}
